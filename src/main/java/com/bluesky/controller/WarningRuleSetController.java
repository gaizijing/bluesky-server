...
import com.bluesky.service.WarningRuleSetService;
...
@Tag(name = "预警规则集")
@RestController
@RequestMapping("/warning-rule-sets")
@RequiredArgsConstructor
public class WarningRuleSetController {

    private final WarningRuleSetService service;
    ...
    @Operation(summary = "发布预警规则集")
    public Result<Map<String, Object>> publish(@PathVariable String id) {
        return Result.success(service.publish(id));
    }

    @PostMapping("/{id}/enable-llm")
    @Operation(summary = "启用/关闭 LLM 解读")
    public Result<Map<String, Object>> enableLlm(@PathVariable String id, @RequestParam boolean enabled) {
        return Result.success(service.enableLlm(id, enabled));
    }
